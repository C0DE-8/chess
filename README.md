...
If the backend host is also on Vercel, disable Socket.IO and let the app use REST move submission plus short polling:

```env
VITE_SOCKET_ENABLED=false
```

Vercel serverless deployments do not provide a stable persistent WebSocket or Engine.IO session. A dedicated Node host such as Render, Railway, Fly.io, or a VPS is the better deployment target for true Socket.IO realtime.
